  ...
  <div class="outer">
    ...
    <div class="content">
      ...
      <div class="action">
        <div style="float: right">
          <a
            class="delete"
            v-if="
              this.userinfo.userid === this.$store.state.user.userinfo.userid
            "
            @click="deleteComment"
          >
            <span>删除</span>
      ...
      <a>
        <span
          style="float: right"
          v-if="commentlv2.length > 0 && commentlv2.length % 10 == 0"
          >查看更多</span
        >
      </a>
    </div>
...
export default {
  ...
  methods: {
    ...
    async getComments() {
      let comment_p = 0;
      if (
        this.$store.getters.getBlogCommentCount
          .get(this.comment.blogId)
          .lv2.get(this.comment.id) != null
      ) {
        comment_p = this.$store.getters.getBlogCommentCount
          .get(this.comment.blogId)
          .lv2.get(this.comment.id);
      }
    ...
    async deleteComment() {
      this.$Modal.confirm({
        title:
          this.commentlv2.length > 0
            ? "<p>删除评论后，评论下的所有回复都会被删除</p>"
            : "<p>确认删除吗？</p>",
        content: "",
        onOk: async () => {
