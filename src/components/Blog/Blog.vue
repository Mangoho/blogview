  ...
  <div style="margin-bottom: 10px">
    <Card class="blog">
      ...
      <div class="b1">
        ...
        <div class="b4" v-html="content">
          <span v-show="content_filed.isover">
            <a
              @click="handleFold(false)"
              v-show="content_filed.maxLen != content_filed.haystack.length"
            >
        ...
        <ImgViewer
		  v-if="blog.picture!=''"
          :urlList="blog.picture"
        ></ImgViewer>
		<!--
        <VedioPlayer
          v-if="blog.mediaType === 2"
          :url="
            'http://' +
              this.$host +
              '/static/vedio/' +
              blog.urlList.substring(1, blog.urlList.length - 1)
          "
        ></VedioPlayer>
        <div v-if="blog.mediaType === 3">
          <router-link :to="'/live/' + blog.userid">
            <Icon type="ios-videocam-outline" />{{
              userinfo.username + "的直播"
            }}
          </router-link>
          <span
            v-if="liveinfo != null && blog.date === liveinfo.startdate"
            style="color: #19be6b"
            >（正在直播）</span
          >
      ...
      <BlogBottom :blog="blog" @Comment="handleComment"></BlogBottom>
      <!--<Comment></Comment>-->
...
export default {
  ...
  computed: {
    content() {
	  if(this.blog.forward_content!=null)
		this.content_filed.haystack=this.blog.forward_content+'<p/>';
    ...
    time() {
      return dateformat(this.blog.loadtime);
    }
  },
  methods: {
    handleComment() {
    ...
    handleFold(fold) {
      this.content_filed.maxLen = fold
        ? this.content_filed.mLen
        : this.content_filed.haystack.length;
    },
    async getUserInfo() {
      let res = await userApi.getInfo(this.blog.userid);
      this.userinfo = res;
      this.load = true;
    },
    loadingok() {
