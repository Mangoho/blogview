<template>
  <div class="comments">
    ...
    <div class="orderby">
      <a
        :style="{ fontWeight: orderBy == 0 ? 'bold' : 'normal' }"
        @click="orderBy = 0"
      >
        按热度
      </a>
      ...
      <a
        :style="{ fontWeight: orderBy == 1 ? 'bold' : 'normal' }"
        @click="orderBy = 1"
      >
        按时间
      </a>
...
export default {
  ...
  methods: {
    async getData() {
      let comment_p = 0;
      if (
        this.$store.getters.getBlogCommentCount != null &&
        this.$store.getters.getBlogCommentCount.get(this.blogId) != null
      ) {
