...
export default {
  ...
  methods: {
    async getFocusOrFans(type) {
      let res = await userApi.getFocusOrFansList(
      ...
      if (type == 10) {
        ...
        this.scrollheight =
          this.focusList.length >= 5 ? 400 : this.focusList.length * 80;
      }
      if (type == 20) {
        this.fansList = res.data;
        this.scrollheight =
          this.fansList.length >= 5 ? 400 : this.fansList.length * 80;
      }
    },
